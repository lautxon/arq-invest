  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(20)].map((_, i) => (
          <div key={i} className="absolute w-1 h-1 bg-arq-gold-400/30 rounded-full animate-pulse"
      ...
      <div className="container-custom relative z-10 text-center">
        <div className="max-w-5xl mx-auto">
          <div className="hero-subtitle inline-flex items-center gap-2 bg-arq-gold-500/20 border border-arq-gold-500/30 rounded-full px-6 py-2 mb-8">
            ...
            <span className="text-arq-gold-300 text-sm font-medium tracking-wide">Proyectos Premium en Buenos Aires</span>
          </div>
          ...
          <div className="hero-cta flex flex-col sm:flex-row gap-4 justify-center items-center">
            <a href="#proyectos" className="btn-gold flex items-center gap-3 group">
              Explorar Proyectos
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </a>
            <button onClick={() => setIsVideoOpen(true)} className="btn-secondary text-white border-white/30 hover:bg-white/10 flex items-center gap-3">
          ...
          <div className="hero-subtitle grid grid-cols-2 md:grid-cols-4 gap-8 mt-20 pt-12 border-t border-white/10">
            {[
              ...
              <div key={index} className="text-center">
                <div className="text-4xl md:text-5xl font-display font-bold text-arq-gold-400 mb-2">{stat.value}</div>
                <div className="text-arq-stone-400 text-sm tracking-wide">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
