  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-5xl mx-4 bg-arq-navy-950 rounded-lg overflow-hidden shadow-2xl">
        ...
        <div className={`relative h-[500px] bg-gradient-to-br ${slides[currentSlide].bg} transition-all duration-700`}>
          <div className="absolute inset-0 overflow-hidden">
            {[...Array(30)].map((_, i) => (
              <div key={i} className="absolute w-1 h-1 bg-white/20 rounded-full animate-pulse"
                style={{ top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, animationDelay: `${Math.random() * 3}s`, animationDuration: `${2 + Math.random() * 2}s` }} />
            ))}
          </div>
          <div className="relative h-full flex flex-col items-center justify-center text-center px-8">
            ...
            <h2 className="text-4xl md:text-5xl font-display font-bold text-white mb-4">{slides[currentSlide].title}</h2>
            <p className="text-xl text-arq-gold-400 mb-6 font-medium">{slides[currentSlide].subtitle}</p>
            <p className="text-arq-stone-300 text-lg max-w-xl whitespace-pre-line leading-relaxed">{slides[currentSlide].description}</p>
            <div className="flex gap-2 mt-12">
              {slides.map((_, index) => (
                <div key={index} className={`h-1 rounded-full transition-all duration-500 ${index === currentSlide ? 'w-8 bg-arq-gold-400' : index < currentSlide ? 'w-4 bg-arq-gold-400/50' : 'w-4 bg-white/20'}`} />
              ))}
            </div>
          </div>
        </div>
        <div className="bg-arq-navy-950 px-6 py-4">
          <div className="w-full h-2 bg-arq-stone-700 rounded-full mb-4 overflow-hidden">
            <div className="h-full bg-arq-gold-500 transition-all duration-100" style={{ width: `${progress}%` }} />
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => setIsPlaying(!isPlaying)} className="w-12 h-12 rounded-full bg-arq-gold-500 flex items-center justify-center hover:bg-arq-gold-600 transition-colors">
                {isPlaying ? <Pause className="w-5 h-5 text-white" /> : <Play className="w-5 h-5 text-white ml-1" />}
              </button>
              <button onClick={() => { setCurrentSlide(0); setProgress(0); setIsPlaying(true) }} className="w-10 h-10 rounded-full bg-arq-stone-700 flex items-center justify-center hover:bg-arq-stone-600 transition-colors">
                <RotateCcw className="w-4 h-4 text-white" />
              </button>
              <div className="text-arq-stone-400 text-sm font-mono">{Math.floor((progress / 100) * 15)}s / 15s</div>
            </div>
            <div className="text-arq-stone-400 text-sm">Slide {currentSlide + 1} de {totalSlides}</div>
          </div>
        </div>
      </div>
    </div>
